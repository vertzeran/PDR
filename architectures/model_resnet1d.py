...
class ResNet1D(nn.Module):
    def __init__(self, num_inputs, num_outputs, block_type, group_sizes, base_plane=64, output_block=None,
                 zero_init_residual=False, gyro_fusion=False, downsampling=False, filter=0, **kwargs):
        ...
        self.filter = filter
        if gyro_fusion==True:
            self.inference_func = self.infer_error
        else:
            self.inference_func = self.infer
        if self.filter:
            self.weights = torch.zeros(num_inputs,num_inputs,self.filter)
            for out_ch in range(num_inputs):
...
class ResNet1D_v2(nn.Module):
    def __init__(self, num_inputs, num_outputs, block_type, group_sizes, base_plane=64, output_block=None,
                 zero_init_residual=False, downsampling=False, filter=0, **kwargs):
