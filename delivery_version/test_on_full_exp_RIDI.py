...
from scripts.create_segments_for_WDE_RIDI import get_segments_from_exp, rotate_traj_to_minimize_error
import torch
...
def test_on_list(exp_list, info, traj_folder, res18model, dataset):
    file_names = []
    PCA_tarj_errors = []
    PCA_tarj_lengths = []
    PCA_errors = []
    ...
    for exp in exp_list:
        errors = test_on_exp(window_size=info["training_window_size"], wind_size_for_heading_init=1000,
                             ...
                             outputfolder=traj_folder, res18model=res18model, add_quat=info["add_quat"],
                             dataset=dataset)
