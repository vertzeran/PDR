...
def get_dir_to_analyze_TRC_1(dataset_location, mode):
    if mode == 'text':
        dir_to_analyze = join(dataset_location, 'texting')
    elif mode == 'pocket':
        dir_to_analyze = join(dataset_location, 'pocket')
    elif mode == 'swing':
        dir_to_analyze = join(dataset_location, 'swing')
    else:
        raise 'invalide mode'
...
def create_segments(params):
    ...
    dir_to_analyze = get_dir_to_analyze_TRC_1(dataset_location=dataset_location,
                                              mode=mode)
    ...
    segments_train, AHRS_results_segments_list_train = \
        get_segments_from_person_list_SBG(person_list=list_of_train_persons,
                                          dir_to_analyze=dir_to_analyze,
                                          window_size=window_size, shuffle=False,
                                          wind_size_for_heading_init=wind_size_for_heading_init,
                                          logger=logger,
                                          traj_length_limit=traj_length_limit
                                          )
    ...
    #                                wind_size_for_heading_init=wind_size_for_heading_init,
    #                                logger=logger,
    #                                calc_heading_fix=heading_fix,
    #                                heading_fix_initialization_time=heading_fix_initialization_time)
    segments_test, AHRS_results_segments_list_test = \
        get_segments_from_person_list_SBG(person_list=list_of_test_persons,
                                          dir_to_analyze=dir_to_analyze,
                                          window_size=window_size, shuffle=False,
                                          wind_size_for_heading_init=wind_size_for_heading_init,
                                          logger=logger
    ...
    X_train, Y1_train, Y2_train = get_xy_pairs(window_size, train_segments_for_WDE, add_quat=add_quat)
    ...
    if create_validation_set:
        X_validation, Y1_validation, Y2_validation = get_xy_pairs(window_size, validation_segments_for_WDE,
                                                                  add_quat=add_quat)
    X_test, Y1_test, Y2_test = get_xy_pairs(window_size, test_segments_for_WDE, add_quat=add_quat)
