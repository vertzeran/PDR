...
from delivery_version.utils.Functions import rotate_trajectory, construct_traj
...
def get_segments_from_exp_list_RIDI(exp_list, window_size, shuffle=True, use_GT_att=False,
                                    wind_size_for_heading_init=750, logger=None
                                    ):
    segment_list = []
    initial_WD_angles_list = []
    if not use_GT_att:
...
def get_AHRS_results_for_exp(exp: Classes.AhrsExp):
...
def get_tain_test_dir(mode):
    if mode == 'text':
        train_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Text - Train'
        test_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Text - Test'
    elif mode == 'pocket':
        train_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Pocket - Train'
        test_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Pocket - Test'
    elif mode == 'bag':
        train_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Bag - Train'
        test_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Bag - Test'
    elif mode == 'body':
        train_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Body - Train'
        test_dir = '/data/Datasets/Navigation/RIDI_dataset_train_test/RIDI - Body - Test'
    else:
        raise 'invalide mode'
...
def get_segments_for_WDE(segments: Classes.AhrsExp, AHRS_results_segments_list: Classes.AHRS_results):
    segments_for_WDE = []
...
def create_segments(params):
    ...
    dataset_location = params["TRC_1_dataset_location"]
    wind_size_for_heading_init = params["wind_size_for_heading_init"]
    main_wd_path = os.getcwd()  # '/home/maint/git/walking_direction_estimation'
    dataset = 'RIDI'  # could be: 'RIDI', 'TRC#1',
    create_validation_set = False
    now = datetime.isoformat(datetime.now())
    description = dataset + '_' + mode + '_' + comment
    print(description)
    ...
    train_dir, test_dir = get_tain_test_dir(mode=mode)
    ...
    segments_train, AHRS_results_segments_list_train = \
        get_segments_from_exp_list_RIDI(exp_list=exp_list_train,
    ...
    segments_test, AHRS_results_segments_list_test = \
        get_segments_from_exp_list_RIDI(exp_list=exp_list_test,
                                        window_size=window_size,
                                        shuffle=False,
                                        use_GT_att=False,
                                        wind_size_for_heading_init=wind_size_for_heading_init,
    ...
    X_train, Y1_train, Y2_train = get_xy_pairs(window_size, train_segments_for_WDE,
                                               add_quat=add_quat)
    # check_segmentation(segment_list=train_segments_for_WDE, X=X_train, Y1=Y1_train, Y2=Y2_train)
    if create_validation_set:
        X_validation, Y1_validation, Y2_validation = get_xy_pairs(window_size, validation_segments_for_WDE,
                                                                  add_quat=add_quat)
    X_test, Y1_test, Y2_test = get_xy_pairs(window_size, test_segments_for_WDE,
                                            add_quat=add_quat)
...
if __name__ == '__main__':
    outputfolder = create_segments(comment='test ', window_size=200, mode='text')
