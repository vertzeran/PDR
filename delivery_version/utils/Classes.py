...
class AhrsExp():
    ...
    def PlotAngles(self):
        ...
        Ax_x.set_xlim(self.Time_GT[0], self.Time_GT[-1]), Ax_x.set(title=r"$\phi$", xlabel="Time [sec]", ylabel="[rad]"), Ax_x.grid(True)
    ...
    def check_for_AHRS_results(self):
        ...
        for item in listdir(self.Path):
            if self.FileName.split(sep='.')[0] in item:
                if item[-suffix_length:] == suffix_to_identify_results and item[:-18] == self.FileName.split(sep='.')[0]:
    ...
    def validate_data_using_time_vectors(self, time_IMU, time_GPS, IMU_time_gap_th, GPS_time_gap_th, min_length_th):
        ...
        if self.first_idx_of_time_gap_IMU.shape[0] != 0:
            ...
            print('time gap in IMU in ') # + str(time_IMU[self.first_idx_of_time_gap_IMU[0] + 1] - time_IMU[0]) + ' [sec]')
        else:
            IMU_idx = list(range(time_IMU.shape[0]))
        ...
        if self.first_idx_of_time_gap_GPS.shape[0] != 0:                                   ##    |
            ...
            print('time gap in GPS in ' )# + str(time_GPS[GPS_idx][self.first_idx_of_time_gap_GPS[0] + 1] - time_GPS[GPS_idx][0]) + ' [sec]')
...
class AI_IMU_DS_Exp(AhrsExp):
    def __init__(self, path=None, initial_dir='/home/maint/Eran/AHRS/AI_dataset/DataForAiIntegration/'):
        ...
        if acc_time.shape[0] != gyro_time.shape[0]:
            self.NumberOfSamples_IMU = min([acc_time.shape[0], gyro_time.shape[0]]) #  gyro and acc are asynchronized ...
            if acc_time.shape[0] > gyro_time.shape[0]:
                acc_x.pop()
                acc_y.pop()
                acc_z.pop()
...
class Euler_angles_plot():
    ...
    def add_est(self,name,time,roll,pitch,heading):
        self.estimates[name] = {"time": time, "pitch": pitch, "roll": roll, "heading": heading, "errors_calculated": False}
...
class WDE_performance_analysis():
    def __init__(self, segment: AhrsExp,
                 ...
                 pdr_net=None, use_GT_dl=True, arc_length_for_dl=False):
        ...
        # self.WD_angle_GT = Functions.FoldAngles(WD_angle_GT.mean())
        # self.WD_vector_GT = np.mean(dP_vectors, axis=0)
        pos = self.segment.Pos.arr()
        overall_d_pos = pos[-1] - pos[0]
        self.WD_vector_GT = overall_d_pos[0:2]
        self.initial_WD_angle_GT = 0
        # self.dL = np.linalg.norm(self.WD_vector_GT)
        if self.use_GT_dl:
        ...
        self.Gyro = segment.Gyro
            ...
            # AHRS_results_file_path = join(self.segment.Path, self.segment.FileName.split(sep='.')[0] + suffix)
            # exp_path = join(self.segment.Path, self.segment.FileName)
            # t_est, phi_hat, phi_e, theta_hat, theta_e, psi_hat, psi_e, grv_hat, Rot_hat = \
            #     Functions.read_AHRS_results(AHRS_results_file_path)
            # # segmentation
            # t_start = self.segment.Time_IMU[0].round(11)
            # t_end = self.segment.Time_IMU[-1].round(11)
            # ind_IMU = np.where((t_est.round(11) >= t_start) & (t_est.round(11)<= t_end))
            # self.lin_acc_b_frame = self.segment.Acc.arr() - grv_hat[ind_IMU]
            # self.Rot = Rot_hat[ind_IMU]
            # self.Heading = np.array(psi_hat)[ind_IMU] + self.segment.Psi[0]
    ...
    def res18_direction_pred(self, plot_results=False,data_type = 'LinAcc', device='cpu', add_quat=False):
        ...
        # if data_type == 'RawIMU':
        #     pth_path = self.model_raw_path
        #     acc = self.segment.Acc.arr()
        #     acc_n_frame = Functions.transform_vectors(acc, self.Rot)
        #     gyro = self.Gyro.arr()
        #     tmp1 = acc_n_frame
        #     tmp2 = gyro
        #     batch_of_a_nav = np.hstack((tmp1,tmp2))
        #     batch_of_a_nav = batch_of_a_nav[None,:,:] #adding batch dim
        if self.res18model is None:
        ...
        if plot_results:
            ...
            Ax.axis('equal')
        self.end_pos_error = np.linalg.norm(self.end_pos_est - self.WD_vector_GT)
        self.WD_est_method = 'resnet18-'+data_type
    ...
    def identify_turn(self, plot_results=False, threshold=1.0):
        ...
        if plot_results:
            ...
            if self.turn_identified:
                plt.text(x=self.segment.Pos.arr()[:, 0].min(), y=self.segment.Pos.arr()[:, 1].min(), s='turn_identified')
            Ax.axis('equal')
            plt.show()
