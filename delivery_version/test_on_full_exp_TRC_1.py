import os
import delivery_version.utils.Classes as Classes
...
from delivery_version.utils.Functions import construct_traj
from scripts.create_segments_for_WDE_SBG import get_segments_from_exp,get_dir_to_analyze, get_exp_list, rotate_traj_to_minimize_error
import torch
import json
import scipy.io as sio

def traj_est_using_SP_heading(Exp, window_size, plot_result=False, wind_size_for_heading_init=200, use_GT_att=False,
...
def traj_est_using_PCA(Exp, window_size, plot_result=False, use_GT_att=False,
                              predict_dL_with_PDRnet=False, pdr_net=None):
    if use_GT_att:
        segment_list, initial_WD_angles_list = get_segments_from_exp(exp=Exp,
                                                                     window_size=window_size,
                                                                     use_GT_att=use_GT_att,
...
def traj_est_using_resnet18(Exp, window_size, plot_result=False, use_GT_att=False, res18model=None, device='cpu',
                            model_path=None, add_quat=False,predict_dL_with_PDRnet=False, pdr_net=None):
    if use_GT_att:
        segment_list, initial_WD_angles_list = get_segments_from_exp(exp=Exp,
                                                                     window_size=window_size,
                                                                     use_GT_att=use_GT_att,
    ...
    for i in range(len(segment_list)):
        ...
        analyzied_segments[-1].res18_direction_pred(plot_results=False, data_type='LinAcc', device=device, add_quat=add_quat)
        walking_angle.append(analyzied_segments[-1].WD_angle_est)
        # dL.append(analyzied_segments[-1].segment.calc_dL(window_size=1).sum())
        # dp = segment.Pos.arr()[-1] - p
        # p = segment.Pos.arr()[-1]
...
def test_on_exp(window_size=200, wind_size_for_heading_init=1000, WD_est_method = 'compare_all_methods',
                experiment_path=None, GT=None, outputfolder=None, res18model=None, predict_dL_with_PDRnet=False,
                pdr_net=None, add_quat=False):
    ...
    if WD_est_method == 'PCA':
        ...
        traj_est_using_SP_heading(Exp, window_size, wind_size_for_heading_init=wind_size_for_heading_init,
                                  pdr_net=pdr_net, predict_dL_with_PDRnet=predict_dL_with_PDRnet,
                                  plot_result=True)
        errors_dic = None
    elif WD_est_method == 'inverted_pendulum':
        traj_est_using_inv_pend(Exp, window_size, plot_result=True, wind_size_for_heading_init=wind_size_for_heading_init,pdr_net=pdr_net,predict_dL_with_PDRnet=predict_dL_with_PDRnet)
    elif WD_est_method == 'resnet18LinAcc':
        dL = Exp.calc_dL(window_size)
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        traj, walking_angle, traj_errors, traj_length, error_metrika = traj_est_using_resnet18(Exp, window_size,
                                                                                               plot_result=True,
                                                                                               use_GT_att=use_GT_att,
                                                                                               device=device,
        ...
        est_traj_PCA, _, traj_errors_PCA, traj_length_PCA, error_metrika_PCA = \
            traj_est_using_PCA(Exp, window_size, plot_result=False, use_GT_att=use_GT_att,pdr_net=pdr_net,predict_dL_with_PDRnet=predict_dL_with_PDRnet)
        est_traj_SP_heading, traj_errors_SP_heading, traj_length_SP_heading, error_metrika_SP_heading = \
            traj_est_using_SP_heading(Exp, window_size, wind_size_for_heading_init=wind_size_for_heading_init,pdr_net=pdr_net,predict_dL_with_PDRnet=predict_dL_with_PDRnet,
                                  plot_result=False)
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        est_traj_resnet18, _, traj_errors_resnet18, traj_length_resnet18, error_metrika_resnet18 = \
            traj_est_using_resnet18(Exp, window_size, plot_result=False, use_GT_att=use_GT_att,
                                      device=device, res18model=res18model, pdr_net=pdr_net, model_path=None,predict_dL_with_PDRnet=predict_dL_with_PDRnet,
                                      add_quat=add_quat)
...
def test_on_list(exp_list, info, traj_folder, res18model, add_quat):
    file_names = []
    PCA_tarj_errors = []
    PCA_tarj_lengths = []
    PCA_errors = []
    ...
    for exp in exp_list:
        errors = test_on_exp(window_size=info["training_window_size"], wind_size_for_heading_init=1000,
                             WD_est_method='compare_all_methods', experiment_path=exp,
                             outputfolder=traj_folder, res18model=res18model, add_quat=add_quat)
    ...
    x_common_SP_heading, y_mean_SP_heading, y_std_SP_heading = calc_error_statistics(X=SP_heading_tarj_lengths,
    ...
    x_common_resnet18, y_mean_resnet18, y_std_resnet18 = calc_error_statistics(X=resnet18_tarj_lengths,
    ...
    plt.fill_between(x_common_PCA, y_mean_PCA - y_std_PCA, y_mean_PCA + y_std_PCA, color='gray', alpha=0.2)
    plt.plot(x_common_SP_heading, y_mean_SP_heading, '-', color='green', label='SP heading')
    plt.fill_between(x_common_SP_heading, y_mean_SP_heading - y_std_SP_heading, y_mean_SP_heading + y_std_SP_heading,
                     color='green', alpha=0.2)
    plt.plot(x_common_resnet18, y_mean_resnet18, '-', color='blue', label='resnet18')
    plt.fill_between(x_common_resnet18, y_mean_resnet18 - y_std_resnet18, y_mean_resnet18 + y_std_resnet18,
...
if __name__ == '__main__':
    calculate_on_test_list = True
    sample_percentage = None
    exp_path = None
    opt_folder = '/data/Work/Navigation/MyCopy'
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    predict_dL_with_PDRnet = True
    add_quat = True
    ...
    if predict_dL_with_PDRnet:
        if info["mode"] == 'Swing': # todo: this needs to be fixed ->  add info file to create segments and inherid mode in training
            pdr_path = '/data/Work/Navigation/SBGResults/PDR_net/swing/checkpoints/checkpoint_31.pt'
        if info["mode"] == 'text':
            pdr_path = '/data/Work/Navigation/SBGResults/PDR_net/text/checkpoints/checkpoint_30.pt'
        if info["mode"] == 'pocket':
            pdr_path = '/data/Work/Navigation/SBGResults/PDR_net/pocket/checkpoints/checkpoint_30.pt'
    ...
    if WD_est_method == 'compare_all_methods' or WD_est_method == 'resnet18LinAcc':
        ...
        data_folder = '/home/maint/git/walking_direction_estimation/data/XY_pairs/2023-02-06T15:12:12.735428TRC#1 swing wind_size 200_heading_angle_fix_initialization = 30 sec'
        res18model = None
    ...
    if calculate_on_test_list:
        ...
        if not load_results:
            ...
            test_on_person_list(person_list=test_persons,
                                ...
                                pdr_net=pdr_net,
                                add_quat=add_quat)
            # test_on_list(exp_list=exp_list, info=info, traj_folder=traj_folder, res18model=res18model, add_quat=add_quat)
        else:
        ...
        test_on_exp(window_size=info["training_window_size"], wind_size_for_heading_init=1000,
                    WD_est_method=WD_est_method,
                    experiment_path=exp_path, outputfolder=traj_folder, res18model=res18model, add_quat=add_quat)
        plt.show()