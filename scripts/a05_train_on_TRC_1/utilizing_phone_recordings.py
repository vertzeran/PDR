...
if __name__ == '__main__':
    local_endroid_rec_dir = '/data/android rec'
    sbg_rec_dir = '/data/Datasets/Navigation/SBG-PDR-DATA/swing/22_07_26_swing_ariel_R'
    main_dir = sbg_rec_dir
    exp_imu_file = None#'indoor_output_2022-01-04_18_32_14_yaw_movement.csv'

    if exp_imu_file is None:
        randomize = False
        if randomize:
            list_of_files = glob.glob(join(main_dir, main_dir) + '/*.csv') # without the "ascii-output.txt"
            exp_path = list_of_files[random.randint(0, len(list_of_files) - 1)]
            print(exp_path)
            Exp = Classes.AndroidExp(exp_path)
        else:
            Exp = Classes.AndroidExp(initialdir=main_dir)
            print(Exp.Path + '/' + Exp.FileName)
    else:
        exp_path = join(main_dir, exp_imu_file)
        print(exp_path)
        Exp = Classes.AndroidExp(exp_path)
    ...
    # Exp.SegmentScenario([0, 5])
    AAE_AHRS = AtitudeEstimator(Ka=0.001, coor_sys_convention=Exp.Frame)
    t0 = time.time()
    if plot_grv and not plot_euler:
        grv_est, RotHat = AAE_AHRS.run_exp(exp=Exp, visualize=plot_grv or plot_euler, return_grv=True, return_euler=False,
                                       save_results_to_file=False)
    elif plot_euler and not plot_grv:
        phi_hat, phi_e, theta_hat, theta_e, psi_hat, psi_e = AAE_AHRS.run_exp(exp=Exp, visualize=plot_grv or plot_euler, return_grv=False, return_euler=True,
                         save_results_to_file=False)
    ...
    Pn = Pn - Pn[0, :]
    if plot_lin_acc_b_frame or plot_lin_acc_n_frame or plot_lin_acc_u_frame:
