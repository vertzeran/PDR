...
def prepare_data_for_training(params):
    ...
    main_wd_path = "/home/maint/git/walking_direction_estimation/"#os.getcwd() #'/'
    now = datetime.isoformat(datetime.now())
    now = now.replace(":", "_")
    # description = params["dataset"] + '_' + params["mode"] + '_' + 'wind_size_' + str(params["window_size"]) + params["comment"]
    print(params["comment"])
    outputfolder = join(main_wd_path, 'data', 'XY_pairs', now + params["comment"])
    mkdir(outputfolder)
    ### info file
    info_dict = params
    ...
    X_test_SZ, Y1_test_SZ, Y2_test_SZ = get_XY_from_exp_list(exp_list=exp_list_test,
                                                                   ...
                                                                   logger=logger)
    ##
    X_train = np.concatenate((X_train_RIDI, X_train_SZ), axis=0)
    X_test = np.concatenate((X_test_RIDI, X_test_SZ), axis=0)
    Y1_train = np.concatenate((Y1_train_RIDI, Y1_train_SZ), axis=0)
    Y1_test = np.concatenate((Y1_test_RIDI, Y1_test_SZ), axis=0)
    Y2_train = np.concatenate((Y2_train_RIDI, Y2_train_SZ), axis=0)
    Y2_test = np.concatenate((Y2_test_RIDI, Y2_test_SZ), axis=0)
