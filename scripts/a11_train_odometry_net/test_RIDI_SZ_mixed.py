...
def test_on_list(exp_list, params, traj_folder, res18model_WDE, res18model_dL=None, dL_source='GT'):
    file_names = []
    PCA_tarj_errors = []
    PCA_tarj_lengths = []
    PCA_errors = []
    ...
    i = 1
    N = len(exp_list)
    predict_dL_with_PDRnet = (dL_source == 'odo_net')
    for exp in exp_list:
        errors = test_on_exp(params, WD_est_method = 'compare_all_methods',
                             experiment_path=exp, GT=None, outputfolder=traj_folder, res18model=res18model_WDE,
                             predict_dL_with_PDRnet=predict_dL_with_PDRnet, pdr_net=res18model_dL,
...
if __name__ == '__main__':
    ...
    dL_opt_folder = '/home/maint/git/walking_direction_estimation/data/optimization_results/2023-03-21T14:41:24.147084optimization results on training odometry solution'
    #'/data/Work/Navigation/MyCopy'
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    predict_dL = True
    WD_est_method = 'compare_all_methods'#'resnet18LinAcc'#'SP_heading'#'PCA'#'compare_all_methods'
    WDE_info_file_path = join(WDE_opt_folder, 'info_file.json')
    with open(WDE_info_file_path, "r") as f:
        WDE_params = json.loads(f.read())
    dL_info_file_path = join(dL_opt_folder, 'info_file.json')
    with open(dL_info_file_path, "r") as f:
        dL_params = json.loads(f.read())
    dL_model_path = join(dL_opt_folder, dL_params["best_saved_model"])
    if predict_dL:
        dL_model = torch.load(dL_model_path, map_location=lambda storage, location: storage)
        dL_model.to(device)
        dL_model.eval()
    else:
        pdr_net = None
    if WD_est_method == 'compare_all_methods' or WD_est_method == 'resnet18LinAcc':
        data_folder = WDE_params["data_folder"]
        WDE_model_name = WDE_params["best_saved_model"]
        model_path = join(WDE_opt_folder, WDE_model_name)
        res18model = torch.load(model_path)
        res18model.to(device)
    else:
        data_folder = ''
        res18model = None
    # todo: dataset=WDE_params["RIDI_params"]["dataset"]  RIDI_params needs to come automatically from th experiment
    exp_name = 'hao_leg2.csv'
    WDE_params = WDE_params["RIDI_params"]
    _, test_dir = get_train_test_dir(dataset=WDE_params["dataset"], mode=WDE_params["mode"])

    exp_path = join(test_dir, exp_name)
    exp_list = [join(test_dir, item) for item in listdir(test_dir) if
                'AHRS_results' not in item]
    traj_folder = join(dL_opt_folder, 'traj')
    ...
    if calculate_on_test_list:
        ...
        test_on_exp(params=WDE_params, WD_est_method = 'compare_all_methods',
                experiment_path=exp_path, GT=None, outputfolder=traj_folder, res18model=res18model, predict_dL_with_PDRnet=predict_dL,
                pdr_net=dL_model, allign_traj=False, save_n_close_fig=False)
        plt.show()