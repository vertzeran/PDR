...
def get_XY_from_exp_list(exp_list, params, use_GT_att=False, logger=None):
    if params["add_quat"]:
        X = np.zeros((0, params["window_size"], 7))
    else:
        X = np.zeros((0, params["window_size"], 3))
    Y1 = np.zeros((0, 1))
    Y2 = np.zeros((0, 2))
    sample_id = 0
    i=1
    ...
    for item in exp_list:
        ...
        X_exp, Y1_exp, Y2_exp = get_xy_pairs(params["window_size"], processed_exp_segments,
                                                   add_quat=params["add_quat"], heading_angle_fix=False)
        X = np.concatenate((X, X_exp), axis=0)
        # For odometry
        Y1 = np.concatenate((Y1, Y1_exp), axis=0)
        # For WDE
        Y2 = np.concatenate((Y2, Y2_exp), axis=0)
...
def prepare_data_for_training(params):
    ...
    outputfolder = join(main_wd_path, 'data', 'XY_pairs', now + '_' + params["preprocess_title"])
    mkdir(outputfolder)
    ### info file
    info_dict = params
    info_dict['processed_data_folder'] = outputfolder
    ...
    logfile_name = 'data_processing_log.log'
    handler = logging.handlers.WatchedFileHandler(
        os.environ.get("LOGFILE", join(outputfolder, logfile_name)))
    formatter = logging.Formatter(logging.BASIC_FORMAT)
    handler.setFormatter(formatter)
    logger_name = 'data_processing_logger'
    logger = logging.getLogger(logger_name)
    logger.setLevel(os.environ.get("LOGLEVEL", "INFO"))
    logger.addHandler(handler)
    #
    info_message = params["preprocess_comment"]
    logger.info(info_message)
    print(info_message)
    if params["add_quat"]:
        X_train = np.zeros((0, params["window_size"], 7))
        X_test = np.zeros((0, params["window_size"], 7))
    else:
        X_train = np.zeros((0, params["window_size"], 3))
        X_test = np.zeros((0, params["window_size"], 3))
    Y1_train = np.zeros((0, 1))
    Y1_test = np.zeros((0, 1))
    Y2_train = np.zeros((0, 2))
    Y2_test = np.zeros((0, 2))
    # RIDI
    if params["process_RIDI"]:
        info_message = 'Processing RIDI datset, preparing train data'
        logger.info(info_message)
        print(info_message)
        train_dir, test_dir = get_train_test_dir(dataset=params["RIDI_params"]["dataset"], mode=params["RIDI_params"]["mode"])
        info_message = ['train_dir = : ' + train_dir]
        logger.info(info_message)
        print(info_message)
        info_message = ['test_dir = : ' + test_dir]
        logger.info(info_message)
        print(info_message)
        ...
        exp_list_test = [join(test_dir, item) for item in listdir(test_dir) if
                          'AHRS_results' not in item]
        X_train_RIDI, Y1_train_RIDI, Y2_train_RIDI = get_XY_from_exp_list(exp_list=exp_list_train,
                                                           params=params["RIDI_params"],
                                                           use_GT_att=False,
                                                           logger=logger)
        info_message = 'preparing test data'
        logger.info(info_message)
        print(info_message)
        X_test_RIDI, Y1_test_RIDI, Y2_test_RIDI = get_XY_from_exp_list(exp_list=exp_list_test,
                                                        params=params["RIDI_params"],
                                                        use_GT_att=False,
                                                        logger=logger)
        X_train = np.concatenate((X_train, X_train_RIDI), axis=0)
        X_test = np.concatenate((X_test, X_test_RIDI), axis=0)
        Y1_train = np.concatenate((Y1_train, Y1_train_RIDI), axis=0)
        Y1_test = np.concatenate((Y1_test, Y1_test_RIDI), axis=0)
        Y2_train = np.concatenate((Y2_train, Y2_train_RIDI), axis=0)
        Y2_test = np.concatenate((Y2_test, Y2_test_RIDI), axis=0)
    # SZ
    if params["process_SZ"]:
        info_message = 'Processing SZ datset, preparing train data'
        logger.info(info_message)
        print(info_message)
        train_dir, test_dir = get_train_test_dir(dataset=params["SZ_params"]["dataset"], mode=params["SZ_params"]["mode"])
        info_message = ['train_dir = : ' + train_dir]
        logger.info(info_message)
        print(info_message)
        info_message = ['test_dir = : ' + test_dir]
        ...
        exp_list_test = [join(test_dir, item) for item in listdir(test_dir) if
                         'AHRS_results' not in item]
        X_train_SZ, Y1_train_SZ, Y2_train_SZ = get_XY_from_exp_list(exp_list=exp_list_train,
                                                                          params=params["SZ_params"],
                                                                          use_GT_att=False,
                                                                          logger=logger)
        info_message = 'preparing test data'
        logger.info(info_message)
        print(info_message)
        X_test_SZ, Y1_test_SZ, Y2_test_SZ = get_XY_from_exp_list(exp_list=exp_list_test,
                                                                       params=params["SZ_params"],
                                                                       use_GT_att=False,
                                                                       logger=logger)

        X_train = np.concatenate((X_train, X_train_SZ), axis=0)
        X_test = np.concatenate((X_test, X_test_SZ), axis=0)
        Y1_train = np.concatenate((Y1_train, Y1_train_SZ), axis=0)
        Y1_test = np.concatenate((Y1_test, Y1_test_SZ), axis=0)
        Y2_train = np.concatenate((Y2_train, Y2_train_SZ), axis=0)
        Y2_test = np.concatenate((Y2_test, Y2_test_SZ), axis=0)
...
if __name__ == '__main__':
    ...
    outputfolder = prepare_data_for_training(params)
    print('data ready')