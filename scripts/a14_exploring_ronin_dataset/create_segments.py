...
def get_segments_from_exp(exp: Classes.AhrsExp, window_size, sample_id=0, use_gt_att=False,
                          win_size_for_heading_init=750, walking_start_threshold=1.0):
    """
    ...
    """
    # exp.define_walking_start_idx(th=walking_start_threshold)
...
def get_segments_from_exp_no_gt(exp: Classes.AhrsExp, window_size, sample_id=0, use_gt_att=False,
                                win_size_for_heading_init=750, walking_start_threshold=1.0, AHRS_results_path = None):
    """
    Creating a list of segments from the experiment samples.
    Each segment contains window_size samples (~250)
    sample_id is useful if want to append such lists
    ...
    initial_wd_angle = exp.initial_WD_angle_GT
    print('fetching AHRS')
    t_est, lin_acc_b_frame_est, grv_est, rot_est, heading_est = get_ahrs_results_for_exp(exp, path=AHRS_results_path)
    exp.initial_heading = heading_est[exp.index_of_walking_start]
    ...
    while seg_stop_idx <= exp.NumberOfSamples_IMU - 1:
        ...
        seg_start_idx += window_size
        seg_stop_idx += window_size
    return segment_list, ahrs_results_list
...
def get_segments_from_person_list_sbg(person_list, dir_to_analyze, window_size, win_size_for_heading_init=750,
                                      ...
                                      add_quat=False, walking_start_threshold=1.0):
    """
    Same as get_segments_from_exp, but now we append all lists from all files
    ...
    for i_person, person in enumerate(person_list):
        ...
        for exp_path in p_bar:
            ...
            if calc_heading_fix:
                ...
                info_message = f'heading angle fix {exp.heading_fix} is calculated on {join(exp.Path, exp.FileName)}'
                logger.info(info_message)
            ...
            exp_segments, exp_ahrs_results_segments, initial_wd_angles_segment = get_segments_from_exp(
                ...
                walking_start_threshold=walking_start_threshold)
            ...
            for segment, ahrs_results_seg in zip(exp_segments, exp_ahrs_results_segments):
                ...
                x, y1, y2 = get_seg_xy_pair(window_size, segment_for_wde, add_quat=add_quat)
...
def get_seg_xy_pair(window_size, segment_for_wde, add_quat=False):
    ...
    # rotating GT
    if hasattr(segment_for_wde.segment, 'heading_fix') and segment_for_wde.segment.heading_fix != 0:
        walking_dir_vector = Functions.rotate_trajectory(traj=walking_dir_vector,
...
def get_ahrs_results_for_exp(exp: Classes.AhrsExp, path=None):
    suffix = '_AHRS_results.xlsx'
    if path is None:
        ahrs_results_file_path = join(exp.Path, exp.FileName.split(sep='.')[0] + suffix)
    else:
        ahrs_results_file_path = join(path, exp.FileName.split(sep='.')[0] + suffix)
    ...
    if False:
        ...
        ind_imu = None
    if ind_imu is None:
        ind_imu = range(exp.NumberOfSamples_IMU)
...
def create_segments(comment="", window_size=200, mode=None, add_quat=False, heading_fix=False, traj_length_limit=None,
    ...
    x_train, y1_train, y2_train = get_segments_from_person_list_sbg(
        ...
        add_quat=add_quat, traj_length_limit=traj_length_limit, walking_start_threshold=walking_start_threshold)
    ...
    x_test, y1_test, y2_test = get_segments_from_person_list_sbg(
        ...
        win_size_for_heading_init=win_size_for_heading_init, logger=logger, calc_heading_fix=heading_fix,
        add_quat=add_quat, walking_start_threshold=walking_start_threshold)
...
if __name__ == '__main__':
    ...
    add_quat = True
    heading_fix = True
    walking_start_threshold = 0.0
    outputfolder = create_segments(comment=comment, window_size=window_size, mode=mode, add_quat=add_quat,
                                   heading_fix=heading_fix, walking_start_threshold=walking_start_threshold)
    print(outputfolder)
